...
Q value calculation and during move selection, before only STOP_VALUE spaces
are still open at the start of get_move. We check how many spaces are open
only once per get_move, instead of once per UCT call, to save time, even though
...
def alt_get_state_string_after_move(game_state, move, curr_player):
    # Returns the state string after applying move to the board
...
def ord_Q_value(r_next_move, t_next_move, t_total, curr_player):
    """
...
def ord_UCB_choose(game_state, state_string, possible_moves, curr_player, 
    states_seen):
    """
    ...
    for i in range(0, len(possible_moves)):
        ...
        # for child state y
        curr_Q_val = ord_Q_value(r_t_pair[0], r_t_pair[1], t_total, curr_player)
...
def ord_UCT(game_state, curr_player, states_seen):
    """
    game_state: the game_state being examined
    curr_player: the tile of the player whose turn it is to move in this state
    ...
    next_move = ord_UCB_choose(game_state, state_string, valid_moves, curr_player, states_seen)
    next_player = "O"
    if curr_player == "O":
        next_player = "X"
    ...
    # that move ...
    val_next_move = ord_UCT(game_state, next_player, states_seen)
...
def alt_Q_value(r_next_move, t_next_move, t_total, curr_player, 
...
def alt_UCB_choose(game_state, state_string, possible_moves, curr_player, 
    states_seen):
    """
    game_state: the game_state being examined
    state_string: the string representation of the game_state
    ...
    best_move = -1
    for i in range(0, len(possible_moves)):
        new_state, box_captured, gives_free_move = alt_get_state_string_after_move(
            game_state, possible_moves[i], curr_player)
        r_t_pair = states_seen[new_state] # this has the list [r_y, t_y] 
        # for child state y
        curr_Q_val = alt_Q_value(r_t_pair[0], r_t_pair[1], t_total, curr_player,
            box_captured, gives_free_move)
        if curr_Q_val > curr_max:
            # in this case, the possible_moves[i] has a better score than 
            # the other moves checked so far
...
def alt_UCT(game_state, curr_player, states_seen):
    """
    game_state: the game_state being examined
    curr_player: the tile of the player whose turn it is to move in this state
    states_seen: the set of seen states, mapped to the each state's r and t 
    ...
    Updates states_seen appropriately for the current game_state based on the 
    results of the successive calls to UCT.
    In Q_value, adds CAPTURE_BOX_CONST when a box is captured and subtracts 
    GIVE_FREE_CONST when a free move is given.
    Returns the r value of the current move = the r value of the next move
    chosen by UCB_choose (which is then used by the previous calls to 
    recursively update states_seen).
    """
    ...
    # if we have reached this point, there are valid moves to be made
    # we can save time by passing in board and state_string to UCB_choose
    next_move = alt_UCB_choose(game_state, state_string, valid_moves, curr_player, states_seen)
    next_player = "O"
    if curr_player == "O":
        next_player = "X"
    # next tile is now set to the tile of the player whose turn is next
    ...
    # ... and then use a recursive call to UCT to find the r value of 
    # that move ...
    val_next_move = alt_UCT(game_state, next_player, states_seen)
    # ... and then update the r and t values of the current move accordingly
    states_seen[state_string][0] = (states_seen[state_string][0] * 
        states_seen[state_string][1] + val_next_move)/(states_seen[state_string][1] + 1.0)
    states_seen[state_string][1] = states_seen[state_string][1] + 1.0
...
def get_move(game_state, curr_player):
    ...
    # by our call to UCT
    state_copy = get_game_state_copy(game_state)
    open_spaces = count_open_spaces(state_copy)

    if open_spaces > STOP_VALUE:
        # call alt_UCT
        alt_UCT(state_copy, curr_player, imp_seen[curr_player])
    else:
        # call ordinary UCT
        ord_UCT(state_copy, curr_player, imp_seen[curr_player])
    ...
    curr_best = [-1, -1, -1, -1]
    if open_spaces > STOP_VALUE:
        # we should add/subtract appropriately for box captures and giving free moves
        if curr_player =="X":
            curr_max_r = -1
            for move in poss_moves:
                post_move, captures_box, gives_free_move = alt_get_state_string_after_move(
        ...
        # we do not add/subtract for box captures and giving free moves
        if curr_player =="X":
            curr_max_r = -1
            for move in poss_moves:
